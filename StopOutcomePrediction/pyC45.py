...
def train(training_obs,training_cat,xmldir):
    ...
    for i in range(0,len(training_obs)):
        ...
        for j in range(len(attrs_names)):
            data[j].append(training_obs.iloc[i,j])
...
def predict(xmldir,testing_obs):
    ...
    prediction=[]
    attrs_names=list(testing_obs.columns.values)
    for i in range(0,len(testing_obs)):
        
        answerlist=decision(root,testing_obs.iloc[i],attrs_names,1)
